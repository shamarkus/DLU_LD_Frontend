...
char* recursiveNameCheck(char* outputFileName, int fileIncrement){
	if(fileIncrement == 100){
		return outputFileName;
	}
	else if (access(outputFileName, F_OK) != 0) {
		return outputFileName;	
	}
	...
	if(!fileIncrement){
		...
		memset(outputFileName + strlen(outputFileName) - 6, 0, 6);
	}
	...
	return recursiveNameCheck(outputFileName,++fileIncrement);
}
int main(int argc,char** argv){
	...
	if(inputParams->fileC > 1){
		char* outputFileName[MAX_STRING_SIZE];
		if(!strlen(argv[6])){
			sprintf(outputFileName,"%s%s",inputParams->dirPath,GENERAL_OUTPUT_NAME);
		}
		else{
			sprintf(outputFileName,"%s%s.%s",inputParams->dirPath,argv[6],CSV_SUFFIX);
		}
		//Check for pre-existing recursively
		strcpy(outputFileName,recursiveNameCheck(outputFileName,0));

		makeCombinedOutput(inputParams,outputFileName);
